...
| **D** | Vitrine ou pedido aberto | **Presencial ou híbrida** | ✅ **No v1**, liberado por confirmação bilateral — ver §8.5. Estados extras de agendamento e falta |
...
✅ **Os quatro entram no lançamento.** O quarto — compromisso com hora marcada — não tem API que confirme presença, então usa um mecanismo diferente e igualmente rigoroso: **confirmação bilateral dos dois lados dentro do app** (§8.5), o mesmo padrão já usado em produto físico sem rastreio.
...
| **4. Compromisso com hora marcada** — evento, palestra, live, gravação presencial | **ninguém** *(se ele também postar, soma-se a linha 1)* | A presença dele, na data e hora combinadas | **Confirmação bilateral:** os dois confirmam no app que o serviço aconteceu. **Se um não confirmar em 7 dias, vale a confirmação do outro** | A marca usa o registro do evento por **12 meses** |
...
### 8.5 Tipo D (presencial e híbrido) 🟢

> 🔴 **O presencial entra no v1. A regra de liberação por API vale para publicação digital; presença física usa o mecanismo certo para o que ela é: confirmação bilateral.**
>
> A regra *"o dinheiro é liberado na publicação confirmada por API"* nasceu para o caso em que existe uma publicação — um post, um vídeo. Presença física não é uma publicação, então não tenta usar a ferramenta errada para o trabalho errado. **O mecanismo correto é o que o próprio produto já usa em produto físico sem rastreio (§8.0.4): os dois lados confirmam, com incentivo econômico dos dois para dizer a verdade** — a marca só confirma se o serviço aconteceu porque é o que libera o pagamento que ela já reteve; o criador só confirma porque é o que libera o dinheiro dele. **Se um mentir sozinho, o outro não confirma, e vira disputa com prazo — nunca a palavra de um só lado decidindo.**
>
> Isso não é exceção à regra do escrow — é a mesma regra (*"nada libera dinheiro sem uma confirmação verificável"*), aplicada ao evento que existe: comparecimento, não postagem.
